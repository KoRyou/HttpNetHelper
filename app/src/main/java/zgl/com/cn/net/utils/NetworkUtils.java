...
public class NetworkUtils {
    ...
    public static boolean isAvailableByPing() {
        ...
        if (result.errorMsg != null) {
            Log.d("网络是否可用 errorMsg", result.errorMsg);
        }
        if (result.successMsg != null) {
            Log.d("网络是否可用 successMsg", result.successMsg);
        }
        return ret;
    }
     ...
     */
    public static void setDataEnabled(boolean enabled,Context context) {
        try {
            TelephonyManager tm = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
    ...
    public static String getDomainAddress(final String domain) {
        try {
            /*ThreadPoolExecutor pool = new ThreadPoolExecutor(1,3,10, TimeUnit.SECONDS,null);
            Future<String> fs = pool.submit(new Callable<String>() {
                @Override
